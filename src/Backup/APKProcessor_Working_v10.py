...
class APKProcessor:
    ...
    def harden_and_notify(self, job: Job):
        ...
        if temp_apk_path.exists():
            ...
            shutil.move(str(temp_apk_path), str(final_apk_path))
        ...
        try:
            ...
            keystore_public_path = public_output_dir / \
                f"uploads/{job.domain}/app/apk/{job.id}.keystore"
            shutil.copy(keystore, keystore_public_path)

            base_url = os.getenv('PUBLIC_DOMAIN', self.base_url).rstrip('/')
            keystore_url = f"{base_url}/hardened/{job.id}.keystore"
            ...
            def send_callback_and_cleanup(url, data, job_id, temp_paths):
                try:
                    requests.post(url, json=data, timeout=15)
                    print(f"[JOB {job_id}] Callback sent")
                except Exception as e:
                    print(f"[JOB {job_id}] Callback failed (ignored): {e}")
                finally:
                    for p in temp_paths:
                        if p and p.exists():
                            try:
                                ...
                                pass
                    ##print(f"[JOB {job_id}] Temp files cleaned up")
            temp_paths = [temp_file, job_folder, temp_apk_path]
            Thread(target=send_callback_and_cleanup, args=(job.callback_url, result, job.job_id, temp_paths), daemon=True).start()
